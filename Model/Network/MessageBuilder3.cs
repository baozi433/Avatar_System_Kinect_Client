...
{
    public class MessageBuilder3
    {
        public OscMessage KinectStatus()
        {
            var address = String.Format("/Data", 2);
            return new OscMessage(address, 0f, 0f, 0f);
        }

        public OscMessage BuildJointMessage(Body body, CameraSpacePoint[] cameraSpacePoint, int index)
        {
            var address = "/Data";
            return new OscMessage(address, index, cameraSpacePoint[0].X,  cameraSpacePoint[0].Y,  cameraSpacePoint[0].Z,  body.Joints[JointType.SpineBase].TrackingState.ToString(), 
